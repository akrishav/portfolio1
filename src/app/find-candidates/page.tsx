    ...
    <div className="flex min-h-screen bg-[#F8F9FA] text-[#1F1F1F] font-sans">
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${isSidebarCollapsed ? "pl-16" : "pl-[288px]"}`}>
        <header className="h-16 bg-white border-b border-[#DFEAF2] px-8 flex items-center justify-between sticky top-0 z-20 shadow-xs">
          <div>
            <h1 className="text-lg font-medium text-[#1F1F1F]">Find Candidates</h1>
            <p className="text-xs text-[#797979]">Search and shortlist candidates across sources</p>
          </div>
        ...
        <main className="p-8 space-y-6 max-w-7xl">
          <div>
            <h2 className="text-2xl font-medium text-[#1F1F1F]">Find Candidates</h2>
            <p className="text-xs text-[#797979] mt-1">Search internal and external sources by skill, location and experience.</p>
          </div>

          {/* Empty Selection Box matching trial prototype Screenshot 6 */}
          <div className="bg-white border border-dashed border-[#DFEAF2] rounded-2xl p-16 flex flex-col items-center justify-center text-center space-y-4 shadow-[0_4px_4px_-2px_rgba(24,39,75,.08)]">
            <div className="w-16 h-16 rounded-full bg-[#CCEFDC] text-[#00AE52] flex items-center justify-center">
              <Search className="w-8 h-8 text-[#00AE52]" />
            </div>
            <div className="max-w-md space-y-1">
              <h3 className="text-base font-semibold text-[#1F1F1F]">Start a search</h3>
              <p className="text-xs text-[#797979] leading-relaxed">
                Try searching by a skill, job title, or location to surface matching candidates across your connected sources.
              </p>
            </div>
          </div>
        </main>
